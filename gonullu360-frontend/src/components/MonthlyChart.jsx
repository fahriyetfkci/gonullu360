...
export default function MonthlyChart({
  ...
  const getYearlyChartData = () => {
    if (!allYearsData?.data) {
      return [];
    }

    return allYearsData.data.map((yearData) => {

      if (filter === 'genel') {
        return {
          name: String(yearData.year),
          Genel: yearData.total,
        };
      }
      ...
      if (filter === 'bölge') {
        return {
          name: String(yearData.year),
          [selectedCity]: yearData.region?.[selectedCity] || 0,
        };
      }

      return {
        name: String(yearData.year),
        Erkek: yearData.gender?.Erkek || 0,
        Kadın: yearData.gender?.Kadın || 0,
      };
    });
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {viewMode === 'aylık' && (
            ...
            >
              {(allYearsData?.data || []).map(item => <option key={item.year} value={item.year}>{item.year}</option>)}
            </select>
          )}
      ...
      {chartData.length === 0 ? <div className="dashboard-empty-chart"><strong>Grafik verisi bulunamadı</strong><span>Seçilen dönem için gönüllü kaydı yok.</span></div> : <ResponsiveContainer width="100%" height={250}>
        <LineChart data={chartData} margin={{ top: 14, right: 24 }}>
          <CartesianGrid
            strokeDasharray="3 3"
            stroke="#f0f0f0"
          />
          ...
          <XAxis
            ...
            axisLine={false}
            tickLine={false}
            interval={viewMode === 'yıllık' ? 'preserveStartEnd' : 0}
          />
