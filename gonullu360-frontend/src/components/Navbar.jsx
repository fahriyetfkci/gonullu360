...
export default function Navbar({ user, notifications }) {
  ...
  useEffect(() => {
    ...
    getNotifications()
      .then((result) => {
        if (active) setManagerNotifications(result?.notifications || []);
      })
      .catch(() => {
        if (active) setManagerNotifications([]);
      });
  ...
  const displayedUser = user || getStoredUser() || { name: 'Yönetici', role: 'ADMIN', photo: null };
  const displayedRole = String(displayedUser.role).toUpperCase() === 'ADMIN' ? 'Yönetici' : displayedUser.role;
  ...
  const handleNotificationClick = async (notification) => {
    if (notification.read !== 0) return;
    try {
      await markNotificationRead(notification.id);
      setManagerNotifications((current) => current.map((item) => (
        item.id === notification.id ? { ...item, read: 1 } : item
      )));
    } catch {
      // Backend yalnızca oturumdaki yöneticinin kendi bildirimini günceller.
    }
  };
        ...
        >
          ...
          {[
            ...
            <button
              ...
              type="button"
              onClick={() => {
                if (item.label === 'Çıkış Yap') logout();
                setShowProfileMenu(false);
              }}
              style={{
                width: '100%',
          ...
          {safeNotifications.length === 0 ? (
            ...
            safeNotifications.map((notification) => (
              <div
                ...
                style={{
                  cursor: notification.read === 0 ? 'pointer' : 'default',
                  padding: '12px 16px',
                  borderBottom: '1px solid #f0f0f0',
                  backgroundColor:
                    notification.read === 0
