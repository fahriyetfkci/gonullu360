...
function FormBuilderPage({ onPreview, onOpenPublished, onManageForms, onBack }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        setSaveState("saving");
        const saved = await saveDraft(schema, revision.current);
  ...
  async function handlePublish() {
    ...
    try {
      await saveNow();
      const result = await publishForm(schema);
      setPublished(result);
      setSaveState("published");
      setMessage(`S\xFCr\xFCm ${result.version} ba\u015Far\u0131yla yay\u0131nland\u0131.`);
    } catch (error) {
      setSaveState("error");
      setMessage(error.response?.data?.error || "Form yay\u0131nlan\u0131rken bir hata olu\u015Ftu.");
    }
  }
  ...
  />)}
    ...
    try {
      ...
      setMessage(error.response?.data?.error || "Taslak kaydedilirken bir hata oluştu.");
              ...
              <span>✓</span><div><strong>Taslağı Kaydet</strong><small>Formu yayınlamadan sakla</small></div><b>›</b>
            </button>
